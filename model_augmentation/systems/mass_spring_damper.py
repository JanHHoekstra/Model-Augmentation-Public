...
class Msd_2dof(System_deriv):
    ...
    def deriv(self, x, u):
        ...
        dz1 = z2
        dz2 = (
            -(self.k1 + self.k2) / self.m1 * z1
            + self.k2 / self.m1 * z3
            - (self.c1 + self.c2) / self.m1 * z2
            + self.c2 / self.m1 * z4
            - self.a1 / self.m1 * np.power((z1), 3)
            + self.a2 / self.m1 * np.power((z3 - z1), 3)
            + 1 / self.m1 * u[0]
        )
        dz3 = z4
        dz4 = (
            -self.k2 / self.m2 * (z3 - z1)
            - self.c2 / self.m2 * (z4 - z2)
            - self.a2 / self.m2 * np.power((z3 - z1), 3)
            + 1 / self.m2 * u[1]
        )
...
class Msd_ndof(System_deriv):
    def __init__(self, n, m, k, c, a, d=None, dt=0.02, output_ix=None, input_ix=None):
        ...
        self.d = self.cast_parameter_to_array(d, self.n)
    ...
    def deriv(self, x, u):
        ...
        if self.nx <= 2:
            dx[0] = x[1]
            dx[1] = (
                -(self.k[0]) / self.m[0] * x[0]
                - (self.c[0]) / self.m[0] * x[1]
                - self.a[0] / self.m[0] * np.power((x[0]), 3)
                - self.d[0] / self.m[0] * np.power((x[1]), 3)
                + 1 / self.m[0] * u[0]
            )
        else:
            dx[0] = x[1]
            dx[1] = (
                -(self.k[0] + self.k[1]) / self.m[0] * x[0]
                + self.k[1] / self.m[0] * x[2]
                - (self.c[0] + self.c[1]) / self.m[0] * x[1]
                + self.c[1] / self.m[0] * x[3]
                - self.a[0] / self.m[0] * np.power((x[0]), 3)
                + self.a[1] / self.m[0] * np.power((x[2] - x[0]), 3)
                - self.d[0] / self.m[0] * np.power((x[1]), 3)
                + self.d[1] / self.m[0] * np.power((x[3] - x[1]), 3)
                + 1 / self.m[0] * u[0]
            )
        ...
        for i in range(1, self.n - 1):
            ...
            F_i = self.k[i] * d_i + self.c[i] * der_d_i + self.a[i] * np.power(d_i, 3) +  self.d[i] * np.power(der_d_i, 3)
            Fi_ = (
                self.k[i + 1] * di_
                + self.c[i + 1] * der_di_
                + self.a[i + 1] * np.power(di_, 3)
                + self.d[i + 1] * np.power(der_di_, 3)
            )
        ...
        if self.nx > 2:
            ...
            dx[-1] = (
                ...
                - self.a[self.n - 1] / self.m[self.n - 1] * np.power((xn - xn_), 3)
                + 1 / self.m[-1] * u[-1]
            )
