...
C_bar_bla = np.append(C_bar_bla, [0,0])[np.newaxis]
...
ANN_state_block = Static_ANN_Block(nz=nxd+nu, nw=nxd, n_nodes_per_layer=64, net=zero_init_feed_forward_nn, activation=torch.nn.Identity) # linear activation function
# ANN_state_block = Static_ANN_Block(nz=nxd+nu, nw=nxd, n_nodes_per_layer=64, net=zero_init_feed_forward_nn, activation=torch.nn.Tanh) # nonlinear activation function
...
# ------------- Save fit system -----------------
model_file_name = "msd_{3}dof_mixed_interconnection_e{0}_nf{1}_batch_size{2}".format(epochs, nf, batch_size, dof)
