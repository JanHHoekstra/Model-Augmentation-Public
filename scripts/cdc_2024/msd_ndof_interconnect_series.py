...
ANN_state_block = Static_ANN_Block(nz=nxd+nu+ny, nw=nxd, n_nodes_per_layer=64, net=zero_init_feed_forward_nn, activation=torch.nn.Tanh)
ANN_output_block = Static_ANN_Block(nz=nxd+nu+ny, nw=ny, n_nodes_per_layer=64, net=zero_init_feed_forward_nn, activation=torch.nn.Tanh)
...
interconnect.connect_block_signals(ANN_state_block, ["u", physical_output_model_block], [])
...
interconnect.connect_block_signals(ANN_output_block, ["u", physical_output_model_block], ["y"])
...
# ------------- Save fit system -----------------
model_file_name = "msd_{3}dof_series_interconnection_e{0}_nf{1}_batch_size{2}".format(epochs, nf, batch_size, dof)
