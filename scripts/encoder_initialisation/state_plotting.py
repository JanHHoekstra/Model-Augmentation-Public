...
# std_x = np.std(baseline_x_test, axis=0) # type: ignore
# test_data_with_x = System_data_with_x(y = test_data.y, u = test_data.u, x = baseline_x_test / std_x)
...
if flag_print_rmse:
    print("\n------------ RMSE ----------------")
    for rmse, name in zip(test_rms_list, fit_sys_name_list):
        print(name + ": " + str(rmse))
    print("--------------------------------\n")
...
if flag_plot_box_plot:
    ...
    plt.rcParams["font.family"] = "Times New Roman"
    scaling = 6 / 10
    fig1 = plt.figure(figsize=[8.9 * scaling, 20.5 * scaling])  # type: ignore

    # get default color list
    prop_cycle = plt.rcParams["axes.prop_cycle"]
    colors = prop_cycle.by_key()["color"]
    x_ticks = encoder_initialisation_types
    for i in range(nx):
        per_x_errors = []
        for error in error_list:
            per_x_errors.append(error[:, i])
        array_per_x_errors = np.array(per_x_errors).T
        plt.subplot(nx, 1, i + 1)
        # plt.boxplot(array_per_x_errors, tick_labels=encoder_initialisation_types)
        violin_parts = plt.violinplot(
            array_per_x_errors, vert=True, showmeans=True, showextrema=True, widths=0.9
        )
        for color, pc in zip(colors, violin_parts["bodies"]):  # type: ignore
            pc.set_color(color)
            pc.set_edgecolor("black")
        violin_parts["cmeans"].set_colors(colors)  # type: ignore
        violin_parts["cmaxes"].set_colors(colors)  # type: ignore
        violin_parts["cmins"].set_colors(colors)  # type: ignore
        violin_parts["cbars"].set_colors(colors)  # type: ignore

        plt.xticks([1, 2], labels=x_ticks)
        plt.tight_layout()
        plt.ylabel("error")
    ...
    if save_flag:
        ...
        fig_file_name = "box_plot_state_errors"
        plt.savefig(
...
if flag_plot_simulation:
    scaling = 12 / 10
    fig1 = plt.figure(figsize=[8.9 * scaling, 3.5 * scaling])  # type: ignore

    for i in range(nx):
        plt.subplot(1, nx, i + 1)
        plt.plot(xft.detach()[:, i], label="System")

    for error, name in zip(error_list, fit_sys_name_list):
        for i in range(nx):
            plt.subplot(1, nx, i + 1)
            plt.plot(error[:, i], label=name)
    plt.legend()
    ...
    if save_flag:
        ...
        fig_file_name = "fitted_models_state_pred_error"
        plt.savefig(
            os.path.join(model_folder_path, fig_file_name + ".svg"), format="svg"
        )
        plt.savefig(
